        ...
        return (
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 relative ">
              ...
              <div
                className={`absolute inset-0 -m-8 opacity-10 bg-gradient-to-r from-${palette.textTitle.split("-")[1]}-500 via-${palette.textDot.split("-")[1]}-500 to-${palette.textTitle.split("-")[1]}-500 blur-2xl`}
              ></div>

              {/* Bagian Teks & Informasi */}
              <div className="flex-1 relative z-10 ">
                <h2
                  className={`text-3xl font-extrabold tracking-tighter ${palette.textTitle} drop-shadow-sm`}
                >
                  {item.name}
                </h2>
                ...
                <p className="mt-5 text-sm text-gray-400 flex items-center gap-3">
                  <span className="font-medium text-gray-100">
                    Developed by{" "}
                    <span className={`font-semibold ${palette.textTitle}`}>
                      {item.details.developer}
                    </span>
                  </span>
                  <span className={`${palette.textDot} font-black`}>
                    &bull;
                  </span>
                  <span className="font-medium text-gray-100">
